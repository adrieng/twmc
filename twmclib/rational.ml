...
let on p q =
  ...
  let p =
    ...
    | Pat v1, Pat v2 ->
       ...
       in
       let v_len =
         Sigs.Int.lcm (Word.weight v1) (Word.length v2)
...
let compare_per p1 p2 =
  ...
  | (Ext (Zero | Omega) | Pat _), _ ->
     Sigs.Compare.int (tag_to_int p1) (tag_to_int p2)

let compare p q =
  if equal p q then 0
  else
    Sigs.Compare.pair Word.compare compare_per
      (p.u, p.v)
      (q.u, q.v)
